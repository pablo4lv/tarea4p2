...
TGaleria crearTGaleria(TFecha fecha){
    ...
    nueva->futuras = NULL;
    nueva->fecha = fecha;
    nueva->visitaDia = crearTVisitaDia(nueva->fecha, MAX_GRUPOS_VISITA_DIA);
...
void avanzarAFechaTGaleria(TGaleria galeria, TFecha fecha){
    ...
    TFecha aux = copiarTFecha(fecha);
    galeria->activas = obtenerExposicionesActivas(unidas2, aux);
    galeria->finalizadas = obtenerExposicionesFinalizadas(unidas2, aux);
